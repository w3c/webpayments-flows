...
Payer<-[#green]>UAM: Select <b><color:red>Card</color></b> Payment App/Instrument
...
note left
	Card Details from UnionCardResponse
	PAN, [Name], [Expiry], CSC, [BillingAddress], [MobilePhoneNo])
end note
...
Alt
	UA->Payee: Card Details
Else
	UA->Payee: Encrypt(Card Details)
...
Payee-\MPSP: Authorise (Card Details)


Group Optional Secondary Authentication
	MPSP<->CPSP: Send Authorisation SMS 

	note over Payee
		Card Details from UnionPayResponse
		PAN, [Name], [Expiry], CSC, [Billing Address], MobilePhoneNo
	end note
	...
	UA->Payee: Card Details + AuthCode
	Payee-\MPSP: Authorise (Card Details + AuthCode)
